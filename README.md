...
## Abstract
We tested a quantum computer vs. a classical computer at playing rock paper, and scissors.  We wanted to see if a completely random values playing rock paper scissors, would beat a classical computer which has patterns in its "random"  algorithm.   We applied Hadamard gates to 2 quibits, and then saw that the probabilities of the outputs were equal.  Then we put the values into a program,  only taking in 3 of the equal probabilities, and ignoring the remaining one.  After that we made the classical computer input a random number corresponding to rock paper or scissors.  We ran rock paper scissors many times,  and recorded the amount of times classical computers won, quantum computers won, and how many ties there were. 

## Introduction
Why is random better than preplanned? 
...
When we have found the values of the quibits that correspond with either Rock, Paper, Or Scissors we match the results against a classical computers result.  The classical computer uses its "random" algorithm to determine if it chooses Rock Paper or Scissors.   The results are compared with each other, and after repeating the calculations an output of how many times the classical computer vs.  the quantum computer is outputed. 
...
## Future Work
Future work would include making the code's loop loop many times, and then save the values outputed in a string to make the program faster.  Future work would also include testing the project on a quantum computer, and outputing many values at once.  The classical computer could be trying to beat the quantum computer, having a premade algorithm to try and guess what the quantum comptuter would output.    
