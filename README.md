...
## Abstract
We tested a quantum computer vs. a classical computer at playing rock paper, and scissors.  We wanted to see if a completely random values playing rock paper scissors, would beat a classical computer which has patterns in its "random"  algorithm.   We applied Hadamard gates to 2 quibits, and then saw that the probabilities of the outputs were equal.We entered the values into a program,  only taking in 3 of the equal probabilities, and ignoring the remaining one.  After that we made the classical computer input a random number corresponding to rock paper or scissors.  We ran rock paper scissors many times,  and recorded the amount of times classical computers won, quantum computers won, and how many ties there were. 
